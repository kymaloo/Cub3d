...
static int	init_map(t_game *game, char *str)
{
	game->parse->all_file = stock_file(game->mm, str);
	// game->parse->grid = extract_map(game, game->parse->all_file, str);
...
bool all_line_is_valid(t_game *game, char **array)
{
	int	i;
	int	size;
	...
	while (array[i])
	{
		if (check_white_space(array[i] != 0) || char_valid_for_map(game, "01NSEWD\t' '" || strncmp_with_array(array[i], , size)))
		i++;
	}
}
