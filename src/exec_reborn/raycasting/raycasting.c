 ...
 */
static t_collision_infos cast_single_ray(const t_player *player, int screen_x)
{
    t_dda dda;
    ...
    while (1) {
        ...
        if (check_collision(&dda, current_dist)) {
 ...
 */
void raycast(const t_player *player, t_collision_infos *collisions)
{
    int screen_x = 0;
    while (screen_x < SCREEN_WIDTH) {
        collisions[screen_x] = cast_single_ray(player, screen_x);
        screen_x++;
    }
}